...
tools: ['search/codebase', 'usages', 'problems', 'changes', 'testFailure', 'runCommands/terminalSelection', 'runCommands/terminalLastCommand', 'openSimpleBrowser', 'fetch', 'search/searchResults', 'edit/editFiles', 'search', 'new', 'runCommands']
...
```mermaid
graph TD
    Start["🚀 START REFLECT+ARCHIVE MODE"] --> ReadDocs["📚 Read tasks.md, progress.md<br>.github/instructions/copilot-instructions.md"]
    ...
    VerifyImplement -->|"Yes"| LoadReflectMap["🗺️ Load Reflect Map<br>.github/instructions/visual-maps/reflect-mode-map.md"]
    ...
    UserCommand -- "ARCHIVE NOW" --> LoadArchiveMap["🗺️ Load Archive Map<br>.github/instructions/visual-maps/archive-mode-map.md"]
...
```read_file({
  target_file: ".github/instructions/visual-maps/reflect-mode-map.md",
  should_read_entire_file: true
})

read_file({
  target_file: ".github/instructions/visual-maps/archive-mode-map.md",
  should_read_entire_file: true
})
```
...
Example for Level 2:
```read_file({
  target_file: ".github/instructions/Level2/reflection-basic.md",
  should_read_entire_file: true
})
read_file({
  target_file: ".github/instructions/Level2/archive-basic.md",
  should_read_entire_file: true
})
```
