...
**CRITICAL:** All file paths MUST follow `memory-bank-paths.instructions.md` structure.

Required Memory Bank structure at project root:
- `memory-bank/` (root directory)
- `memory-bank/tasks.md` (active task tracking - ephemeral)
...
- `memory-bank/archive/` (archived tasks)

```mermaid
graph TD
    FV["File Verification"] --> CheckFiles["Check Essential Files"]
    CheckFiles --> CheckMB["Check Memory Bank<br>Structure at ROOT"]
    CheckMB --> MBExists{"memory-bank/<br>Exists at Root?"}
    
    MBExists -->|"Yes"| VerifyMB["Verify Memory Bank<br>Files & Subdirs"]
    MBExists -->|"No"| CreateMB["Create memory-bank/<br>at Project Root"]
    
    CreateMB --> CreateSubDirs["Create Subdirectories:<br>creative/, reflection/, archive/"]
    CreateSubDirs --> CreateFiles["Create Core Files:<br>tasks.md, activeContext.md,<br>progress.md, etc."]
    CreateFiles --> VerifyMB
    
    VerifyMB --> CheckSubDirs{"Subdirectories<br>Complete?"}
    CheckSubDirs -->|"No"| CreateSubDirs
    CheckSubDirs -->|"Yes"| CheckCoreFiles{"Core Files<br>Exist?"}
    
    CheckCoreFiles -->|"No"| CreateFiles
    CheckCoreFiles -->|"Yes"| MBCP["Memory Bank<br>Checkpoint"]
    
    MBCP --> FileComplete["File Verification<br>Complete"]
...
3. **Required remediation steps take priority** over any pending mode switches or complexity instructions
