...
graph TD
    ...
    ReadTasks --> ReadAnalytics["Read Analytics Documents<br>memory-bank/analytics/"]
    ReadAnalytics --> ExtractInsights["Extract Key Insights<br>from Analysis Results"]
    
    %% Complexity Level Determination
    ExtractInsights --> CheckLevel{"Determine<br>Complexity Level"}
...
graph TD
    ...
    CheckActive -->|"Yes"| CheckAnalytics{"Analytics documents<br>exist?"}
    
    CheckAnalytics -->|"No"| ErrorAnalytics["ERROR:<br>Return to ANALYZE Mode"]
    CheckAnalytics -->|"Yes"| ReadyPlan["Ready for<br>Planning"]
```

## 🔍 KEY INSIGHTS EXTRACTION PROCESS

Extract and reference analysis findings for planning decisions:

```mermaid
graph TD
    ExtractStart["Extract Key Insights"] --> ReadAnalytics["Read Analytics<br>Documents"]
    ReadAnalytics --> IdentifyFindings["Identify Key<br>Findings"]
    IdentifyFindings --> CategorizeInsights["Categorize Insights<br>by Planning Impact"]
    CategorizeInsights --> CreateSummary["Create Insights<br>Summary"]
    
    CreateSummary --> TechInsights["Technical<br>Insights"]
    CreateSummary --> UserInsights["User/Business<br>Insights"]
    CreateSummary --> RiskInsights["Risk/Challenge<br>Insights"]
    CreateSummary --> SolutionInsights["Solution<br>Recommendations"]
    
    TechInsights & UserInsights & RiskInsights & SolutionInsights --> PlanningInput["Input to<br>Planning Process"]
    
    style ExtractStart fill:#4da6ff,stroke:#0066cc,color:white
    style CreateSummary fill:#6c5ce7,stroke:#5f3dc4,color:white
    style PlanningInput fill:#26de81,stroke:#20bf6b,color:white
```
...
- [Technical constraints/requirements identified]

### User/Business Insights
- [User needs/pain points from analysis]
- [Business requirements/goals identified]
...
- [Recommended approaches from analysis]
- [Priority features/components identified]
