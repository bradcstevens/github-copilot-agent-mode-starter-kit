...
function generateCollectionReadme(collection, collectionId) {
  ...
  if (collection.display?.ordering === "alpha") {
    items.sort((a, b) => {
      const titleA = extractTitle(path.join(ROOT_DIR, a.path));
      const titleB = extractTitle(path.join(ROOT_DIR, b.path));
...
try {
  ...
  const instructionsDir = path.join(ROOT_DIR, "instructions");
  const promptsDir = path.join(ROOT_DIR, "prompts");
  const chatmodesDir = path.join(ROOT_DIR, "chatmodes");
  const collectionsDir = path.join(ROOT_DIR, "collections");
  ...
  writeFileIfChanged(path.join(ROOT_DIR, "README.instructions.md"), instructionsReadme);
  writeFileIfChanged(path.join(ROOT_DIR, "README.prompts.md"), promptsReadme);
  writeFileIfChanged(path.join(ROOT_DIR, "README.chatmodes.md"), chatmodesReadme);
  writeFileIfChanged(path.join(ROOT_DIR, "README.collections.md"), collectionsReadme);
